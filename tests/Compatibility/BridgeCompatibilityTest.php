...
final class BridgeCompatibilityTest extends TestCase
{
    public function testRealSymfonyApplicationSnapshot(): void
    {
        ...
        $securityTrees = array_values(array_filter($configurationBundles, static fn (mixed $bundle): bool => \is_array($bundle) && 'security' === ($bundle['alias'] ?? null)));
        self::assertCount(1, $securityTrees);
        $securityTree = $securityTrees[0]['tree'] ?? null;
        self::assertIsArray($securityTree);
        $securityChildren = array_column(\is_array($securityTree['children'] ?? null) ? $securityTree['children'] : [], null, 'name');
        $firewalls = $securityChildren['firewalls'] ?? null;
        self::assertIsArray($firewalls);
        $firewallPrototype = $firewalls['prototype'] ?? null;
        self::assertIsArray($firewallPrototype);
        $firewallAliases = $firewallPrototype['aliases'] ?? null;
        self::assertIsArray($firewallAliases);
        self::assertSame('custom_authenticators', $firewallAliases['custom_authenticator'] ?? null);
        $firewallChildren = array_column(\is_array($firewallPrototype['children'] ?? null) ? $firewallPrototype['children'] : [], null, 'name');
        $customAuthenticators = $firewallChildren['custom_authenticators'] ?? null;
        self::assertIsArray($customAuthenticators);
        self::assertSame('array', $customAuthenticators['type'] ?? null);
        $customAuthenticatorAccepts = $customAuthenticators['accepts'] ?? null;
        self::assertIsArray($customAuthenticatorAccepts);
        self::assertFalse($customAuthenticatorAccepts['scalar'] ?? true);
        $customAuthenticatorPrototype = $customAuthenticators['prototype'] ?? null;
        self::assertIsArray($customAuthenticatorPrototype);
        self::assertSame('scalar', $customAuthenticatorPrototype['type'] ?? null);
        ...
        $searchController = array_column($stimulusControllers, null, 'name')['search'] ?? null;
        self::assertIsArray($searchController);
        self::assertStringEndsWith('assets/controllers/search_controller.js', \is_string($searchController['sourcePath'] ?? null) ? $searchController['sourcePath'] : '');
        self::assertFalse($searchController['vendor'] ?? true);
        self::assertArrayNotHasKey('actions', $searchController);
