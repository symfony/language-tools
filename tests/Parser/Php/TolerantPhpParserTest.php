...
final class TolerantPhpParserTest extends TestCase
{
    ...
    public function testQueriesReceiverCallsVisibleVariablesAndReceiverTypes(): void
    {
        ...
        self::assertSame(PhpReceiverMatch::Matches, $document->matchReceiver($withName, 'Psr\\Log\\LoggerInterface'));
        self::assertSame(PhpReceiverMatch::Matches, $document->matchReceiver($dispatch, 'Symfony\\Component\\Messenger\\MessageBusInterface'));
        self::assertSame(PhpReceiverMatch::Unrelated, $document->matchReceiver($debug, 'Psr\\Log\\LoggerInterface'));
        self::assertSame(PhpReceiverMatch::Matches, $document->matchReceiver($dispatch, 'Symfony\\Component\\Messenger\\MessageBus', 'Symfony\\Component\\Messenger\\MessageBusInterface'));
        self::assertSame(PhpReceiverMatch::Unrelated, $document->matchReceiver($withName, 'Symfony\\Component\\Messenger\\MessageBus', 'Symfony\\Component\\Messenger\\MessageBusInterface'));
        self::assertSame(PhpReceiverMatch::Unrelated, $document->matchReceiver($withName, 'Log\\LoggerInterface', 'LoggerInterface'));
        self::assertSame(PhpReceiverMatch::Unrelated, $document->matchReceiver($debug, 'Psr\\Log\\LoggerInterface', 'Symfony\\Component\\Messenger\\MessageBusInterface'));
        self::assertSame(PhpReceiverMatch::Unknown, $document->matchReceiver($send, 'Symfony\\Component\\Messenger\\MessageBusInterface'));
        ...
        self::assertSame(['logger', 'bus', 'clock'], array_map(static fn ($variable): string => $variable->name, $document->visibleVariables($tick->methodStartOffset)));
        self::assertSame(['logger', 'clock'], array_map(static fn ($variable): string => $variable->name, $document->visibleVariables($send->methodStartOffset)));
