...
final class ConsoleInputReceiverResolver
{
    ...
    {
        $variables = $php->receiverVariables($call);
        if (array_any($variables, self::isInputVariable(...))) {
            return true;
        }
        if ([] !== $variables
        ...
        foreach ($php->typedVariables as $variable) {
            if ($call->receiverContext->name !== $variable->name
                ...
                || !self::isInputVariable($variable)
                || !$this->methodContains($source, $variable, $call)
            ) {
                continue;
            }

            return $this->closureCaptures($source, $call, $variable->name);
        }
    ...
    private function methodContains(string $source, PhpTypedVariable $variable, PhpMethodCall $call): bool
    {
        if (null === $variable->scopeStartOffset) {
            return false;
        }
        $parametersOpen = strpos($source, '(', $variable->scopeStartOffset);
