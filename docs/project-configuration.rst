...
* ``kernel``: kernel class name or project-relative application entry point
  booted for runtime analysis, for applications with several kernels;
...
project root, the ``dev`` environment, automatic kernel detection, debug,
runtime indexing and release metadata enabled, a 300-second timeout and
missing-translation diagnostics disabled. Set ``releaseMetadata`` to ``false``
to prevent release metadata network access and skip the installed-branch
support check. When a compatible
...
Applications are discovered per ``composer.json``, and each one boots a single
kernel. When one Composer install ships several kernels, ``kernel`` selects the
one to analyze:
...
point instead, which suits applications whose kernel needs its own bootstrap or
...
The entry point must return the application closure of the Symfony Runtime, as
front controllers generated by Symfony Flex do. Without this setting, the
kernel is detected automatically.

A configured kernel is never replaced by another one: an unknown class, a class
that is not a Symfony kernel, a missing entry point or a kernel that cannot be
created is reported as a runtime failure. Run ``symfony-lsp check --verbose``
to see the cause.

Only one kernel is analyzed at a time. Check each application separately with
``symfony-lsp check --kernel=CLASS`` followed by the paths that belong to it,
and switch kernels in the editor with the client command described in the
editor guides.
