...
function through a function import, an alias or a fully qualified call.
...
``;`` comments are ignored. Unsaved resource changes are available immediately,
and changes made by external tools are picked up while the server is running.
